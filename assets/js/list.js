$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 1; i <= num; i++) {
                var tpl = "<li><a href='list.html{0}'>{1}</a></li>"
                if (i == page) {
                    tpl = "<li class='active'><a href='list.html{0}'>{1}</a></li>"
                }
...
function setMenu() {
    ...
    $("#order-x").attr("href", "list.html" + location.search.replace(current, "order="))
        // $("#order-h").attr("href", "list.html"+location.search.replace(current,"order=h"))
    $("#order-l").attr("href", "list.html" + location.search.replace(current, "order=l"))
    $("#order-m").attr("href", "list.html" + location.search.replace(current, "order=m"))
    $("#order-h").attr("href", "list.html" + location.search.replace(current, "order=h"))
}
